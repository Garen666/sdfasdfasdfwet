...
class help_index extends Engine_Class {
    ...
    public function process() {
        ...
        $path = PROJECT_PATH."/docs/usermanual/$f";

        if (!file_exists($path) || is_dir($path)) {
            $path = PROJECT_PATH."/docs/usermanual/page1.html";
        }
        $line = file_get_contents($path, true);
        $this->setValue('text', $line);

        $this->_xml = simplexml_load_string(file_get_contents(PackageLoader::Get()->getProjectPath().'/contents/help/help.xml'));
