...
class install extends Engine_Class {
    ...
    private function _commentModesInEngineMode() {
        ...
        $development = "PackageLoader::Get()->setMode('development');";
        $debug = "PackageLoader::Get()->setMode('debug');";
        $textArray = explode("\n", $text);
        if (!in_array("//$development", $textArray))
        $text = str_replace($development, "//".$development, $text);
...
Engine::Get()->setConfigField('site-language', '$lang');";
