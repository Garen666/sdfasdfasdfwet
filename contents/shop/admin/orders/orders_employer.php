...
class orders_employer extends Engine_Class {
    ...
    public function process() {
        try {
            ...
            Engine::GetHTMLHead()->setTitle('Исполнители заказа '.$order->makeName());
            ...
            if ($this->getControlValue('ok')) {
                try {
                    ...
                    foreach ($args as $key => $value) {
                        if (preg_match("/^status(\d+)$/ius", $key, $r)) {
                            $index = $r[1];
                            ...
                            $oes = new XShopOrderEmployer();
                            $oes->setOrderid($order->getId());
                            $oes->setManagerid($this->getArgumentSecure('manager'.$index, 'float'));
                            $oes->setSum($this->getArgumentSecure('sum'.$index, 'float'));
                            $oes->setPercent($this->getArgumentSecure('percent'.$index, 'float'));
                            $oes->setRole($this->getArgumentSecure('role'.$index, 'string'));
