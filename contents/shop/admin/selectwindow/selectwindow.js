        ...
        success: function(html) {
            ...
            if (options.productsearch) {
                ...
                $j('#'+windowID+'-product-search-button').click(function (event) {
                    shop_product_search($productSearchInput.val(), function (json) {
                        var html = '';
                        $j(json).each(function (i, e) {
                            ...
                            if (e.image != false ) {
                                html += '<a href="#" class="result-element js-product" rel="'+e.id+'"> <img src = '+ e.image+'></img><span class="id">#'+e.id+'</span> <span class="price">'+$price+' '+e.currencyname+'</span> <span class="availtext '+availClass+'">'+availText+'</span> <span class="name">'+e.name+'&nbsp;</span><span class="clear"></span></a>';
                            }else {
                ...
                // когда меняются категории то выводим товары по выбраной категорие
                $j('#'+windowID+'-product-search-categoryid').change(function(){
                    shop_product_search($j(this).val(), function (json) {

                        $j('.js-product').each(function(){
                            $j(this).remove();
                        })
                        ...
                        $j(json).each(function (i, e) {
                            if (e.image != false ){
                                ...
                                html += '<a href="#" class="result-element js-product" rel="'+e.id+'"> <img src = '+ e.image+'></img><span class="id">#'+e.id+'</span> <span class="price">'+e.price+' '+e.currencyname+'</span> <span class="name">'+e.name+'&nbsp;</span></a>';
                            }else {
                                html += '<a href="#" class="result-element js-product" rel="'+e.id+'"><span class="id">#'+e.id+'</span> <span class="price">'+e.price+' '+e.currencyname+'</span> <span class="name">'+e.name+'&nbsp;</span></a>';
                            }
                        });
                        $j('#'+windowID+'-product-search-result').html(html);
                        $j('#'+windowID + '-loading').hide();
                    });
