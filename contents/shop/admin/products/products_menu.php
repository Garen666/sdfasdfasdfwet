...
class products_menu extends Engine_Class {
    ...
    public function process() {
        try {
            ...
            $this->setValue('canEdit', ($user->isAllowed('products-edit')
            || ($isOwner && $user->isAllowed('products-owner-edit'))));
            ...
            $this->setValue('canViewOrders', $user->isAllowed('products-orders'));
            ...
            $moduleTabArray = Shop_ModuleLoader::Get()->getProductTabArray();
            foreach ($moduleTabArray as $k => $moduleTabInfo) {
                $moduleTabArray[$k]['url'] = Engine::GetLinkMaker()->makeURLByContentIDParam($moduleTabInfo['contentID'], $product->getId());
            }
