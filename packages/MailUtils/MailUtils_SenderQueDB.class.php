 ...
 * Складывает письма в таблицу в базе, а затем отдельным скриптом в cron'e
...
class MailUtils_SenderQueDB implements MailUtils_ISender {
    ...
    public function __construct($package = 'SQLObject') {
        ...
        if (PackageLoader::Get()->getMode('development')) {
            $table = SQLObject_Config::Get()->addTable('mailutils_que', 'MailUtils_XQue');
            ...
            $table = SQLObject_Config::Get()->addTable('mailutils_queattachment', 'MailUtils_XQueAttachment');
            $table->addField('id', "int(11)", 'auto_increment');
            $table->addIndexPrimary('id');
    ...
    public static function ProcessQue($limit = 50, $clearInterval = 168, $package = 'SQLObject') {
        new self($package);
        if (MailUtils_Config::Get()->getSender() instanceof MailUtils_SenderQueDB) {
     ...
     * $inteval указывается в часах и указывает максимальный срок хранения обработанной записи с момента её обработки.
    ...
    public static function ClearQueue($interval = 168) {
        ...
        try {
            ...
            // Ramm: формирование даты в пхп - не оптимизация запроса.При вставке в очередь дата берётся из пхп.
            // Посему и тут мы должны формировать в пхп дабы не напороться на рассинхронизаци часовых поясов
            $date = date('Y-m-d H:i:s', time() - $interval*3600);

            $attachments = new MailUtils_XQueAttachment();
            $attachments->addWhereQuery('(`queid` IN (SELECT `id` FROM `'.$que->getTablename().'` WHERE `status`=\'1\'' . ($interval?" AND `pdate` <= '$date'":'').'))');
            while ($attachment = $attachments->getNext()) {
                unlink(dirname(__FILE__).'/media/mailutils_que/'.$attachment->getFile());
                $attachment->delete();
            }

            SQLObject_Config::Get()->getConnectionDatabase()->query("DELETE FROM `{$que->getTablename()}` WHERE `status`='1'" . ($interval?" AND `pdate` <= '$date'":''));
            SQLObject::TransactionCommit();
        } catch (Exception $e) {
            SQLObject::TransactionRollback();
