...
class StringUtils_Converter {
	 ...
	 * @return string
	 */
	public static function FloatToMoney($numeric,$lang='ua') {
		...
		if ($pos===false){
			...
			if (($pos+1) == strlen($numeric)) $numeric .= '00';
			if (($pos+2) == strlen($numeric)) $numeric .= '0';
	...
	private static function _GetPaperMoney($num,$lang){
		...
		for ($i=0;$i<$len;$i++){
			...
			if ($a==0){
				if (($len-$i-2)>-1){
					if ($num[$len-$i-2]=='1'){
						$res=self::$_Money[$lang]['teens'][$num[$len-$i-1]].' '.self::$_Money[$lang]['teensnames'][$i/3].' '.$res;
					}else
					if (($len-$i-3)>-1){
						if (($num[$len-$i-2]=='0')&&($num[$len-$i-1]=='0')&&($num[$len-$i-3]=='0')&&($i/3!=0))
						{}else
		...
		return mb_strtoupper(mb_substr($res,0,1,'UTF-8'),'UTF-8').mb_substr($res,1,mb_strlen($res,'UTF-8'),'UTF-8');
	}
	...
	private static function _InitMoneyWords($lang){
		if ($lang == 'ua'&&!isset(self::$_Money[$lang])){
			...
			self::$_Money[$lang]['null'] = 'Ноль гривень';
