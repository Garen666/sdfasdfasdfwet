...
class Engine {
    ...
    public static function Initialize() {
        ...
        Events::Get()->addEvent('beforeContentProcess', new Engine_Event_ContentProcess());
        Events::Get()->addEvent('afterContentProcess', new Engine_Event_ContentProcess());
        Events::Get()->addEvent('afterContentRender', new Engine_Event_ContentRender());
        Events::Get()->addEvent('afterEngineFinal', new Events_Event());
        Events::Get()->addEvent('afterQueryDefine', new Events_Event());
        ...
        if (file_exists($engineInitFilePath)) {
            include($engineInitFilePath);
        }

        $contentsFile = PackageLoader::Get()->getProjectPath().'contents.php';
        ...
        self::InitializeEngineMode();
        self::InitializeEngineConfig();
    }

    /**
     * Подгрузить и отработать файл engine.mode.php
     * Метод можно вызвать до инициации Engine, чтобы заранее установить
     * соеденение с БД и определить режим работы Engine.
     *
     * @access public
     * @static
     */
    public static function InitializeEngineMode() {
        $filePath = PackageLoader::Get()->getProjectPath().'engine.mode.php';
        if (file_exists($filePath)) {
            // ВАЖНО: include_once для того, чтобы дважды не include'ить файл
            // сколько раз не вызывай, все равно подключиться только 1 раз
            include_once($filePath);
        }
    }

    /**
     * Подгрузить и отработать engine.config.php
     *
     * @access public
     * @static
     */
    public static function InitializeEngineConfig() {
    ...
    public function execute() {
        ...
        $query->defineContentID($parser->getMatchURL(), $argumentsArray);
        ...
        $this->getResponse()->setBody(self::GetContentDriver()->getString(
    ...
    public function getConfigFieldSecure($field) {
        try {
            return $this->getConfigField($field);
        } catch (Exception $e) {}
        return false;
    }
