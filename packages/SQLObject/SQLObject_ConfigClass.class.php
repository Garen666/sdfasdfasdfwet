...
class SQLObject_ConfigClass {
    ...
    public function process($rebuildIndexes = false) {
        ...
        foreach ($this->_connectionArray as $connection) {
            ...
            $this->_sync->process($rebuildIndexes);
        }
        ...
        foreach ($iteratableFields as $field) {
            ...
            $fdata .= "     */\n";
            $fdata .= "    public function set{$m}($$fieldName, \$update = false) {\$this->setField('$fieldName', $$fieldName, \$update);}\n";
            $fdata .= "\n";
            $fdata .= "    /**\n";
            $fdata .= "     * Filter {$fieldName}{$comment}\n";
            $fdata .= "     * @param $type $$fieldName\n";
            $fdata .= "     */\n";
            $fdata .= "    public function filter{$m}($$fieldName) {\$this->filterField('$fieldName', $$fieldName);}\n";
            $fdata .= "\n";
        }
