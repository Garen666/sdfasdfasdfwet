...
class Compile_Scss_To_Css {
    ...
    public function compile () {
        ...
        $this->getFilesArray($rootDir.'/_css/');
        $this->getFilesArray($rootDir.'/contents/');
    ...
    private function _compilesCssToCss () {
        ...
        foreach ($this->_filesArray as $k => $file) {
            ...
            if (isset($importsFileNameArray[$k])) {
                ...
                // Если в этот файл входят другие, то находим где они лежат, и говорим компилятору, что бы он искал их по данному пути
            ...
            file_put_contents(str_replace('.scss', '.css', $file['filepath']), $compiler->compile(file_get_contents($file['filepath'])));
        }
    ...
    private function getFilesArray($path) {
        ...
        while(($file = readdir($handle))) {
            ...
            if (is_file ($path."/".$file) && $this->_getExtension($file) == "scss" ) {
                $this->_filesArray[str_replace('.scss', '', $file)] = array('filepath' => str_replace('//', '/', $path ."/".$file), 'filename' => $file, 'path' => str_replace('//', '/', $path).'/');
            }
