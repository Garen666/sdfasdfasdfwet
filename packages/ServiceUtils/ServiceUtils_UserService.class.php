...
class ServiceUtils_UserService extends ServiceUtils_AbstractService {
    ...
    public function getUserByLogin($userLogin) {
        return $this->getObjectByField('login', $userLogin);
    }
    ...
    public function getUserByEmail($userEmail) {
        return $this->getObjectByField('email', $userEmail);
    }
    ...
    public function login($login, $password, $cookie = false, $inCrypt = false) {
        ...
        if ($user) {
            $user->setAdate(date('Y-m-d H:i:s'));
            $user->setSdate(date('Y-m-d H:i:s'));
            $user->setIp(@$_SERVER['REMOTE_ADDR']);
            $user->setSid($this->getSessionID());
            $user->update();
            ...
            if ($cookie) {
                ...
                if ($authHost) {
                    ...
                    if (Engine::Get()->getConfigFieldSecure('testing')) {
                        setcookie('authlogin', $user->getLogin(), $cookie_etime, '/');
                        setcookie('authpass', $user->getPassword(), $cookie_etime, '/');
                    } else {
                        setcookie('authlogin', $user->getLogin(), $cookie_etime, '/', '.' . $authHost);
                        setcookie('authpass', $user->getPassword(), $cookie_etime, '/', '.' . $authHost);
                    }
                }
            }
    ...
    public function logout(User $user = null) {
        ...
        if ($user) {
            ...
            if ($authHost) {
                ...
                if (Engine::Get()->getConfigFieldSecure('testing')) {
                    setcookie('authlogin', 0, $cookie_etime, '/');
                    setcookie('authpass', 0, $cookie_etime, '/');
                } else {
    ...
    public function getUser() {
        ...
        // кикаем юзеров
        if (rand(0, 10) == 1) {
            $interval = $this->getSessionTime() * 60;

            $users = new $classname;
            $users->addWhere('sid', '', '!=');
            $users->addWhereQuery('adate <= NOW() - INTERVAL '.$interval.' MINUTE');
