...
class Datasource_Users extends Forms_ADataSourceSQLObject {
    ...
    protected function _initialize() {
        ...
        if (Engine::Get()->getConfigFieldSecure('project-box')) {
            ...
            $field->setName('Процент реферальные / партнерские');
            $this->addField($field);
        }
        ...
        $field->setName(Shop::Get()->getTranslateService()->getTranslateSecure('translate_comment_hidden_administrator'));
        $field->setQuickedit(true);
        $this->addField($field);
        ...
        $field->setName(Shop::Get()->getTranslateService()->getTranslateSecure('translate_subscribe_to_our_newsletter'));
        $this->addField($field);

        if (Engine::Get()->getConfigFieldSecure('project-box')) {
            $field = new Shop_ContentFieldUserInfo('authorid');
            $field->setName('Создал');
            $field->setDataSource(new Datasource_Users());
            $field->setQuickedit(false);
            $field->setEmptyOptionValue(0);
            $this->addField($field);
    ...
    public function insert($fieldsArray) {
        try {
            ...
            try {
                ...
                'shop-history-user-'.$r,
                Shop::Get()->getTranslateService()->getTranslateSecure('translate_added_a_new_user').' #'.$r.' '.$x->getName(),
                $user->getId()
                );
            } catch (Exception $e) {
    ...
    public function update($key, $fieldsArray) {
        try {
            ...
            try {
                ...
                Shop::Get()->getTranslateService()->getTranslateSecure('translate_edit_by_user').' #'.$x->getId().' '.$x->getName()."\n".implode("\n", $diffArray),
                $user->getId()
                );
            } catch (Exception $e) {
    ...
    public function delete($key) {
        ...
        try {
            ...
            try {
                ...
                CommentsAPI::Get()->addComment(
                'shop-history-user-'.$x->getId(),
                Shop::Get()->getTranslateService()->getTranslateSecure('translate_blocked_user').' #'.$x->getId().' '.$x->getLogin(),
                $user->getId()
                );
            } catch (Exception $e) {
