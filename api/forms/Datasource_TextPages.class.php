...
class Datasource_TextPages extends Forms_ADataSourceSQLObject {
    ...
    protected function _initialize() {
        ...
        $field->setName(Shop::Get()->getTranslateService()->getTranslateSecure('translate_page_name'));
        $field->addValidator(new Forms_ValidatorEmpty());
        $this->addField($field);

        $field = new Forms_ContentFieldCKEditor('content');
        $field->setName(Shop::Get()->getTranslateService()->getTranslate('translate_page_content'));
        $this->addField($field);

        $field = new Forms_ContentField('btnname');
        $field->setName(Shop::Get()->getTranslateService()->getTranslateSecure('translate_button_name'));
        $field->addValidator(new Forms_ValidatorEmpty());
        $this->addField($field);
        ...
        $field->setName(Shop::Get()->getTranslateService()->getTranslateSecure('translate_parent_page'));
        ...
        $field->setName(Shop::Get()->getTranslateService()->getTranslateSecure('translate_type_page'));
        $this->addField($field);

        $field = new Forms_ContentField('key');
        $field->setName(Shop::Get()->getTranslateService()->getTranslateSecure('translate_key'));
        $this->addField($field);
    ...
    public function insert($fieldsArray) {
        try {
            ...
            try {
                ...
                Shop::Get()->getTranslateService()->getTranslateSecure('translate_added_a_new_page').' #'.$r.' '.$x->getName(),
                $user->getId()
                );
            } catch (Exception $e) {
    ...
    public function update($key, $fieldsArray) {
        try {
            ...
            try {
                ...
                Shop::Get()->getTranslateService()->getTranslateSecure('translate_edited_text_page').' #'.$textpage->getId().' '.$textpage->getName()."\n".implode("\n", $diffArray),
                $user->getId()
                );
            } catch (Exception $e) {
    ...
    public function delete($key) {
        try {
            ...
            try {
                ...
                CommentsAPI::Get()->addComment(
                'shop-history-textpage-'.$textpage->getId(),
                Shop::Get()->getTranslateService()->getTranslateSecure('translate_removed_page').' #'.$textpage->getId().' '.$textpage->getName(),
                $user->getId()
                );
            } catch (Exception $e) {
