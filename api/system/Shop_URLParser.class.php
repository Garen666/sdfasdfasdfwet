...
class Shop_URLParser implements Engine_IURLParser {
    ...
    public function getMatchURL() {
        ...
        if ($urlLast && !substr_count($urlOriginal, 'admin/') && $urlOriginal != '/install/') {

            // ищем lotType
            try {
                $lotType = Shop::Get()->getLotTypeService()->getLotTypeByURL($urlLast);
                $result = $lotType->makeURL(false);
            ...
            // ищем игру
            try {
                $game = Shop::Get()->getGameService()->getGameByURL($urlLast);
                $result = $game->makeURL(false);
                $this->_matchArray[$url_md5] = $result;
                return $result;
            } catch (Exception $e) {
