...
function softPagination() {
    ...
    if ($j('.js-product-list-group-id').length) {
        ...
        $j(window).scroll(function () {
            ...
            if (!$j('.js-os-stepper .selected').length) { // Если нет степера, то выходим
                return false;
            }
...
function configurePagination( nextLink ) {
    ...
    if ( $j('a[data-rel="prev"]').length ) {
        ...
        $j('.js-os-stepper').prepend('<a href="' + prevUrl + '"id="back" data-rel="prev">&larr;Назад</a>')
    }
